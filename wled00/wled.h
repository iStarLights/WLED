...
#define WLED_DISABLE_2D_FX        // disables 2d effects
#define WLED_DISABLE_SFX          // disables sound effects
#define WLED_ENABLE_SIMPLE_UI
...
WLED_GLOBAL char ntpServerName[33] _INIT("0.pool.ntp.org");   // NTP server to use

// WiFi CONFIG (all these can be changed via web UI, no need to set them here)
WLED_GLOBAL char clientSSID[33] _INIT(CLIENT_SSID);
WLED_GLOBAL char clientPass[65] _INIT(CLIENT_PASS);
WLED_GLOBAL char cmDNS[33] _INIT("istarlights");                             // mDNS address (placeholder, is replaced by wledXXXXXX.local)
...
WLED_GLOBAL byte col[]    _INIT_N(({ 158,5,242,50 }));  // current RGB(W) primary color. col[] should be updated if you want to change the color.
WLED_GLOBAL byte colSec[] _INIT_N(({ 0, 0, 0, 0 }));      // current RGB(W) secondary color
WLED_GLOBAL byte briS     _INIT(128);                     // default brightness

WLED_GLOBAL byte nightlightTargetBri _INIT(0);      // brightness after nightlight is over
WLED_GLOBAL byte nightlightDelayMins _INIT(60);
WLED_GLOBAL byte nightlightMode      _INIT(NL_MODE_SET); // See const.h for available modes. Was nightlightFade
WLED_GLOBAL bool fadeTransition      _INIT(true);   // enable crossfading color transition
WLED_GLOBAL uint16_t transitionDelay _INIT(750);    // default crossfade duration in ms

WLED_GLOBAL byte briMultiplier _INIT(255);          // % of brightness to set (to limit power, if you set it to 50 and set bri to 255, actual brightness will be 127)

// User Interface CONFIG
#ifndef SERVERNAME
WLED_GLOBAL char serverDescription[33] _INIT("iStar Lights");  // Name of module - use default
#else
...
WLED_GLOBAL char alexaInvocationName[33] _INIT("Roof Lights");          // speech control name of device. Choose something voice-to-text can understand
...
WLED_GLOBAL bool arlsForceMaxBri _INIT(true);                    // enable to force max brightness if source has very dark colors that would be black
...
WLED_GLOBAL byte currentTimezone _INIT(6);   // Timezone ID. Refer to timezones array in wled10_ntp.ino
...
WLED_GLOBAL byte overlayMin _INIT(0), overlayMax _INIT(DEFAULT_LED_COUNT);   // boundaries of overlay mode
...
// network time
WLED_GLOBAL bool ntpConnected _INIT(true);
...
WLED_GLOBAL float longitude _INIT(-113.53);
WLED_GLOBAL float latitude _INIT(53.13);
