...
struct ColorOrderMap {
  void add(uint16_t start, uint16_t len, uint8_t colorOrder) {
    if (_count >= ISTAR_MAX_COLOR_ORDER_MAPPINGS) {
      return;
    }
    if (len == 0) {
      return;
  ...
  private:
  uint8_t _count;
  ColorOrderMapEntry _mappings[ISTAR_MAX_COLOR_ORDER_MAPPINGS];
};
    ...
    static void setCCT(uint16_t cct) {
      _cct = cct;
		...
		static void setCCTBlend(uint8_t b) {
			...
			#ifdef ISTAR_MAX_CCT_BLEND
				if (_cctBlend > ISTAR_MAX_CCT_BLEND) _cctBlend = ISTAR_MAX_CCT_BLEND;
			#endif
		}
...
class BusPwm : public Bus {
  ...
  BusPwm(BusConfig &bc) : Bus(bc.type, bc.start) {
    ...
    for (uint8_t i = 0; i < numPins; i++) {
      ...
      #else
      ledcSetup(_ledcStart + i, ISTAR_PWM_FREQ, 8);
...
class BusManager {
  ...
  int add(BusConfig &bc) {
    if (numBusses >= ISTAR_MAX_BUSSES) return -1;
