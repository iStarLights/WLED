...
#define ISTAR_DEBOUNCE_THRESHOLD      50 // only consider button input of at least 50ms as valid (debouncing)
#define ISTAR_LONG_PRESS             600 // long press if button is released after held for at least 600ms
#define ISTAR_DOUBLE_PRESS           350 // double press if another press within 350ms after a short press
#define ISTAR_LONG_REPEATED_ACTION   300 // how often a repeated action (e.g. dimming) is fired on long press on button IDs >0
#define ISTAR_LONG_AP               5000 // how long button 0 needs to be held to activate ISTAR-AP
#define ISTAR_LONG_FACTORY_RESET   10000 // how long button 0 needs to be held to trigger a factory reset
...
void shortPressAction(uint8_t b)
{
  ...
  if (buttonPublishMqtt && ISTAR_MQTT_CONNECTED) {
    char subuf[64];
...
void handleSwitch(uint8_t b)
{
  ...
  if (millis() - buttonPressedTime[b] > ISTAR_DEBOUNCE_THRESHOLD) { //fire edge event only after 50ms without change (debounce)
...
{
  static uint8_t oldRead[ISTAR_MAX_BUTTONS] = {0};
  static float filteredReading[ISTAR_MAX_BUTTONS] = {0.0f};
...
void handleButton()
{
  ...
  for (uint8_t b=0; b<ISTAR_MAX_BUTTONS; b++) {
    #ifdef ESP8266
    ...
    if (isButtonPressed(b)) { //pressed
      ...
      if (millis() - buttonPressedTime[b] > ISTAR_LONG_PRESS) { //long press
        if (!buttonLongPressed[b]) longPressAction(b);
        else if (b) { //repeatable action (~3 times per s) on button > 0
          longPressAction(b);
          buttonPressedTime[b] = millis() - ISTAR_LONG_REPEATED_ACTION; //300ms
        }
      ...
      if (dur < ISTAR_DEBOUNCE_THRESHOLD) {buttonPressedBefore[b] = false; continue;} //too short "press", debounce
      bool doublePress = buttonWaitTime[b]; //did we have a short press before?
      buttonWaitTime[b] = 0;

      if (b == 0 && dur > ISTAR_LONG_AP) { // long press on button 0 (when released)
        if (dur > ISTAR_LONG_FACTORY_RESET) { // factory reset if pressed > 10 seconds
          ISTAR_FS.format();
          clearEEPROM();
          doReboot = true;
        } else {
          ISTAR::instance().initAP(true);
        }
    ...
    if (buttonWaitTime[b] && millis() - buttonWaitTime[b] > ISTAR_DOUBLE_PRESS && !buttonPressedBefore[b]) {
      buttonWaitTime[b] = 0;
      shortPressAction(b);
    }
  }
