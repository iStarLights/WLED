...
void getSettingsJS(byte subPage, char* dest)
{
  ...
  if (subPage == 2)
  {
    ...
    if (ethernetType != ISTAR_ETH_NONE && ethernetType < ISTAR_NUM_ETH_TYPES) {
      for (uint8_t p=0; p<ISTAR_ETH_RSVD_PINS_COUNT; p++) { oappend(","); oappend(itoa(esp32_nonconfigurable_ethernet_pins[p].pin,nS,10)); }
    ...
    oappend(itoa(ISTAR_MAX_BUSSES,nS,10));  oappend(",");
    ...
    oappend(itoa(ISTAR_MAX_COLOR_ORDER_MAPPINGS,nS,10));
    oappend(SET_F(");"));
  ...
  if (subPage == 6)
  {
    ...
    sappends('m',SET_F("(\"sip\")[0]"),(char*)F("ISTAR "));
    olen -= 2; //delete ";
