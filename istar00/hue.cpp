...
void handleHue()
{
  ...
  if (!ISTAR_CONNECTED || hueClient == nullptr || millis() - hueLastRequestSent < huePollIntervalMs) return;
...
void reconnectHue()
{
  if (!ISTAR_CONNECTED || !huePollingEnabled) return;
