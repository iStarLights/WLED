...
function applyCfg()
{
	...
	size();
	localStorage.setItem('istarUiCfg', JSON.stringify(cfg));
}
...
function onLoad() {
	...
	var sett = localStorage.getItem('istarUiCfg');
	...
	pmtLS = localStorage.getItem('istarPmt');
...
function presetError(empty)
{
	...
	try {
		bckstr = localStorage.getItem("istarP");
...
{
	if (fromls) pJson = JSON.parse(localStorage.getItem("istarP"));
	delete pJson["0"];
	var cn = "";
	...
	if (pNum > 0) {
		if (pmtLS != pmt && pmt != 0) {
			localStorage.setItem("istarPmt", pmt);
			pJson["0"] = {};
			localStorage.setItem("istarP", JSON.stringify(pJson));
		}
		pmtLS = pmt;
...
function displayRover(i,s)
{
	...
	d.getElementById('lv').innerHTML = `ISTAR is receiving live ${i.lm} data${sour}`;
...
function selSegEx(s)
{
	...
	// ISTAR internally regards the first selected as mainseg regardless of this as long as any segment is selected
	//obj.mainseg = s;
	requestJson(obj);
}
