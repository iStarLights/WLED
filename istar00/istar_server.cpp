...
void handleUpload(AsyncWebServerRequest *request, const String& filename, size_t index, uint8_t *data, size_t len, bool final){
  ...
  if(!index){
    request->_tempFile = ISTAR_FS.open(filename, "w");
...
void initServer()
{
  ...
  //if OTA is allowed
  if (!otaLock){
    #ifdef ISTAR_ENABLE_FS_EDITOR
     #ifdef ARDUINO_ARCH_ESP32
      server.addHandler(new SPIFFSEditor(ISTAR_FS));//http_username,http_password));
     #else
      server.addHandler(new SPIFFSEditor("","",ISTAR_FS));//http_username,http_password));
     #endif
    #else
...
void serveSettings(AsyncWebServerRequest* request, bool post)
{
  ...
  #ifdef ISTAR_DISABLE_MOBILE_UI //disable welcome page if not enough storage
