#define ISTAR_DEFINE_GLOBAL_VARS //only in one source file, istar.cpp!
#include "istar.h"
#include "istar_ethernet.h"
#include <Arduino.h>

#if defined(ARDUINO_ARCH_ESP32) && defined(ISTAR_DISABLE_BROWNOUT_DET)
#include "soc/soc.h"
#include "soc/rtc_cntl_reg.h"
#endif

/*
 * Main ISTAR class implementation. Mostly initialization and connection logic
 */
...
void WiFiEvent(WiFiEvent_t event)
{
  ...
  char hostname[25] = "istar-";
  #endif
  
  switch (event) {
#if defined(ARDUINO_ARCH_ESP32) && defined(ISTAR_USE_ETHERNET)
...
void ISTAR::loop()
{
  ...
  if (!realtimeMode || realtimeOverride || (realtimeMode && useMainSegmentOnly))  // block stuff if WARLS/Adalight is enabled
  {
    ...
    if (ISTAR_CONNECTED && aOtaEnabled) ArduinoOTA.handle();
    #endif
    handleNightlight();
    handlePlaylist();
    yield();
  ...
  if (doInitBusses) {
    ...
    busses.removeAll();
    uint32_t mem = 0;
    for (uint8_t i = 0; i < ISTAR_MAX_BUSSES; i++) {
...
void ISTAR::enableWatchdog() {
#if ISTAR_WATCHDOG_TIMEOUT > 0
#ifdef ARDUINO_ARCH_ESP32
  esp_err_t watchdog = esp_task_wdt_init(ISTAR_WATCHDOG_TIMEOUT, true);
...
#else
  ESP.wdtEnable(ISTAR_WATCHDOG_TIMEOUT * 1000);
#endif
#endif
}

void ISTAR::disableWatchdog() {
#if ISTAR_WATCHDOG_TIMEOUT > 0
...
void ISTAR::setup()
{
...
#endif
#ifdef ISTAR_ENABLE_DMX //reserve GPIO2 as hardcoded DMX pin
  ...
  for (uint8_t i=1; i<ISTAR_MAX_BUTTONS; i++) btnPin[i] = -1;

  bool fsinit = false;
  DEBUGFS_PRINTLN(F("Mount FS"));
#ifdef ARDUINO_ARCH_ESP32
  fsinit = ISTAR_FS.begin(true);
#else
  fsinit = ISTAR_FS.begin();
#endif
  if (!fsinit) {
  ...
  {
    strcpy_P(cmDNS, PSTR("istar-"));
    sprintf(cmDNS + 5, "%*s", 6, escapedMac.c_str() + 6);
  }
  if (mqttDeviceTopic[0] == 0) {
    strcpy_P(mqttDeviceTopic, PSTR("istar/"));
    sprintf(mqttDeviceTopic + 5, "%*s", 6, escapedMac.c_str() + 6);
  }
  if (mqttClientID[0] == 0) {
    strcpy_P(mqttClientID, PSTR("ISTAR-"));
...
#endif
      ISTAR::instance().disableWatchdog();
      DEBUG_PRINTLN(F("Start ArduinoOTA"));
    });
    ArduinoOTA.onError([](ota_error_t error) {
      // reenable watchdog on failed update
      ISTAR::instance().enableWatchdog();
    });
    if (strlen(cmDNS) > 0)
...
void ISTAR::initAP(bool resetAP)
{
...
bool ISTAR::initEthernet()
{
  ...
  if (ethernetType == ISTAR_ETH_NONE) {
    return false;
  }
  if (ethernetType >= ISTAR_NUM_ETH_TYPES) {
...
void ISTAR::handleConnection()
{
  ...
  if (now < 2000 && (!ISTAR_WIFI_CONFIGURED || apBehavior == AP_BEHAVIOR_ALWAYS))
    return;
  ...
  if (apActive) {
    ...
    if (stac != stacO) {
      ...
      DEBUG_PRINTLN(stac);
      if (!ISTAR_CONNECTED && ISTAR_WIFI_CONFIGURED) {        // trying to connect, but not connected
        if (stac)
  ...
  if (!Network.isConnected()) {
    ...
    if (now - lastReconnectAttempt > ((stac) ? 300000 : 18000) && ISTAR_WIFI_CONFIGURED) {
...
// else blink at 1Hz when ISTAR_CONNECTED is false (no WiFi, ?? no Ethernet ??)
// else blink at 2Hz when MQTT is enabled but not connected
// else turn the status LED off
void ISTAR::handleStatusLED()
{
  #if STATUSLED
  ...
  ledStatusType = ISTAR_CONNECTED ? 0 : 2;
  if (mqttEnabled && ledStatusType != 2) { // Wi-Fi takes precendence over MQTT
    ledStatusType = ISTAR_MQTT_CONNECTED ? 0 : 4;
  }
  if (ledStatusType) {
