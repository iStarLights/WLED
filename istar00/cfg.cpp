  ...
  */
  ...
  if (fromFS || !ins.isNull()) {
    ...
    for (JsonObject elm : ins) {
      ...
      if (fromFS) {
        ...
        if (mem <= MAX_LED_MEMORY && busses.getNumBusses() <= ISTAR_MAX_BUSSES) busses.add(bc);  // finalization will be done in ISTAR::beginStrip()
      } else {
  ...
  if (!hw_com.isNull()) {
    ...
    for (JsonObject entry : hw_com) {
      if (s > ISTAR_MAX_COLOR_ORDER_MAPPINGS) break;
  ...
  if (!hw_btn_ins.isNull()) {
    ...
    for (JsonObject btn : hw_btn_ins) {
      ...
      if (++s >= ISTAR_MAX_BUTTONS) break; // max buttons reached
    }
    // clear remaining buttons
    for (; s<ISTAR_MAX_BUTTONS; s++) {
      btnPin[s]           = -1;
  ...
  getStringFromJson(mqttDeviceTopic, if_mqtt[F("topics")][F("device")], 33); // "istar/test"
  ...
  getStringFromJson(ntpServerName, if_ntp[F("host")], 33); // "1.istar.pool.ntp.org"
  ...
  if (doInitBusses) return false; // no save needed, will do after bus init in istar.cpp loop
  return (doc["sv"] | true);
}
...
void serializeConfig() {
  ...
  if (ethernetType != ISTAR_ETH_NONE && ethernetType < ISTAR_NUM_ETH_TYPES) {
    JsonArray pins = ethernet.createNestedArray("pin");
    for (uint8_t p=0; p<ISTAR_ETH_RSVD_PINS_COUNT; p++) pins.add(esp32_nonconfigurable_ethernet_pins[p].pin);
  ...
  hw_btn["max"] = ISTAR_MAX_BUTTONS; // just information about max number of buttons (not actually used)
  JsonArray hw_btn_ins = hw_btn.createNestedArray("ins");

  // configuration for all buttons
  for (uint8_t i=0; i<ISTAR_MAX_BUTTONS; i++) {
  ...
  File f = ISTAR_FS.open("/cfg.json", "w");
...
void serializeConfigSec() {
  ...
  File f = ISTAR_FS.open("/wsec.json", "w");
  if (f) serializeJson(doc, f);
  f.close();
  releaseJSONBufferLock();
}
