...
void handleImprovPacket() {
  ...
  //dbgf = ISTAR_FS.open("/improv.log","a");
        ...
        if (packetByte >= packetLen) { //end of packet, check checksum match
          ...
          switch (rpcCommandType) {
            ...
            case ImprovRPCType::Request_State: {
              ...
              if (ISTAR_WIFI_CONFIGURED) improvState = 0x03; //provisioning
...
void sendImprovInfoResponse() {
  ...
  out[lengthSum] = hlen;
  lengthSum += hlen + 1;
  //Use serverDescription if it has been changed from the default "ISTAR", else mDNS name
  bool useMdnsName = (strcmp(serverDescription, "ISTAR") == 0 && strlen(cmDNS) > 0);
