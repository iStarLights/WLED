...
#if ISTAR_FS != LITTLEFS && ESP_IDF_VERSION_MAJOR < 4
...
bool writeObjectToFile(const char* file, const char* key, JsonDocument* content)
{
  ...
  uint32_t pos = 0;
  f = ISTAR_FS.open(file, "r+");
  if (!f && !ISTAR_FS.exists(file)) f = ISTAR_FS.open(file, "w+");
  if (!f) {
...
void updateFSInfo() {
    ...
    #if ISTAR_FS == LITTLEFS || ESP_IDF_VERSION_MAJOR >= 4
    fsBytesTotal = ISTAR_FS.totalBytes();
    fsBytesUsed = ISTAR_FS.usedBytes();
    #else
...
bool handleFileRead(AsyncWebServerRequest* request, String path){
  ...
  if(ISTAR_FS.exists(pathWithGz)){
    request->send(ISTAR_FS, pathWithGz, contentType);
    return true;
  }*/
  if(ISTAR_FS.exists(path)) {
    request->send(ISTAR_FS, path, contentType);
    return true;
  }
  return false;
}
