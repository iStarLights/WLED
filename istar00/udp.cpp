...
#define ISTARPACKETSIZE (41+(MAX_NUM_SEGMENTS*UDP_SEG_SIZE)+0)
...
void notify(byte callMode, bool followUp)
{
  ...
  byte udpOut[ISTARPACKETSIZE];
  WS2812FX::Segment& mainseg = strip.getMainSegment();
  udpOut[0] = 0; //0: istar notifier protocol 1: WARLS protocol
  udpOut[1] = callMode;
  udpOut[2] = bri;
  ...
  notifierUdp.write(udpOut, ISTARPACKETSIZE);
...
void realtimeLock(uint32_t timeoutMs, byte md)
{
  if (!realtimeMode && !realtimeOverride) {
    ...
    // if ISTAR was off and using main segment only, freeze non-main segments so they stay off
...
void handleNotifications()
{
  ...
  // ISTAR nodes info notifications
  if (isSupp && udpIn[0] == 255 && udpIn[1] == 1 && len >= 40) {
    if (!nodeListEnabled || notifier2Udp.remoteIP() == localIP) return;

    uint8_t unit = udpIn[39];
    NodesMap::iterator it = Nodes.find(unit);
    if (it == Nodes.end() && Nodes.size() < ISTAR_MAX_NODES) { // Create a new element when not present
      Nodes[unit].age = 0;
      it = Nodes.find(unit);
    }
  ...
  //istar notifier, ignore if realtime packets active
