...
void handleSettingsSet(AsyncWebServerRequest *request, byte subPage)
{
  ...
  if (subPage == 2)
  {
    ...
    for (uint8_t s=0; s<ISTAR_MAX_BUTTONS; s++) {
    ...
    for (uint8_t s = 0; s < ISTAR_MAX_BUSSES; s++) {
      ...
      // actual finalization is done in ISTAR::loop() (removing old busses and adding new)
    ...
    ColorOrderMap com = {};
    for (uint8_t s = 0; s < ISTAR_MAX_COLOR_ORDER_MAPPINGS; s++) {
    ...
    for (uint8_t i=0; i<ISTAR_MAX_BUTTONS; i++) {
      char bt[4] = "BT"; bt[2] = (i<10?48:55)+i; bt[3] = 0; // button pin (use A,B,C,... if ISTAR_MAX_BUTTONS>10)
      char be[4] = "BE"; be[2] = (i<10?48:55)+i; be[3] = 0; // button type (use A,B,C,... if ISTAR_MAX_BUTTONS>10)
  ...
  if (subPage == 5)
  {
    ...
    if (ntpEnabled && ISTAR_CONNECTED && !ntpConnected) ntpConnected = ntpUdp.begin(ntpLocalPort);
  ...
  if (subPage == 6)
  {
    if (request->hasArg(F("RS"))) //complete factory reset
    {
      ISTAR_FS.format();
      clearEEPROM();
      serveMessage(request, 200, F("All Settings erased."), F("Connect to ISTAR-AP to setup again"),255);
      doReboot = true;
    }
  ...
  #ifdef ISTAR_ENABLE_DMX // include only if DMX is enabled
  if (subPage == 7)
  {
